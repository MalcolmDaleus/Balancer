...
function StreamsTab({ active }: { active: boolean }) {
    ...
    // Two forms: stream info + price update
    ...
    const reset = () => { setSelected(null); setForm(blankStream); setPriceForm(blankPrice); setShowPriceUpdate(false); setError(null); };
    ...
    const activeAmount = (s: RecurringStream) => {
        const active = s.entries?.find(e => e.active && !e.end_date);
        return active ? `$${active.amount.toFixed(2)}` : null;
    };
            ...
            <SplitPane
                ...
                form={
                    <div className="space-y-4">
                        ...
                        {/* Price update section — only when editing an existing active stream */}
                        {selected && !selected.deleted_at && (
                            <div className="border-t border-slate-100 pt-3 dark:border-slate-700">
                                {!showPriceUpdate ? (
                                    <button onClick={() => setShowPriceUpdate(true)}
                                        className="text-xs font-medium text-indigo-500 hover:text-indigo-700 dark:text-indigo-400">
                                    ...
                                    <form onSubmit={handlePriceUpdate} className="space-y-3">
                                        ...
                                        <Field label="Effective From">
                                            <input type="date" required className={inputCls} value={priceForm.start_date} onChange={e => setPriceForm(f => ({ ...f, start_date: e.target.value }))} />
                                        </Field>
...
function RecurringCategoriesTab({ active }: { active: boolean }) {
    ...
    const selectRow = (c: RecurringCategory) => { setSelected(c); setForm({ name: c.name }); setError(null); };
    const reset = () => { setSelected(null); setForm({ name: '' }); setError(null); };
