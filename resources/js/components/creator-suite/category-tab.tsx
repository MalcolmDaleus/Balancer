...
export function CategoryTab({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (selected) {
                await apiFetch(updateUrl(selected.id), { method: 'PUT', body: JSON.stringify(form) });
            } else {
                await apiFetch(storeUrl, { method: 'POST', body: JSON.stringify(form) });
            }
            reset();
            setFetched(false);
        } catch (err: unknown) {
    ...
    const handleDelete = async (c: CategoryRow) => {
        try {
            await apiFetch(deleteUrl(c.id), { method: 'DELETE' });
